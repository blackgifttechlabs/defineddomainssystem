  ...
  return (
    <div className="grid w-full grid-cols-1 items-start gap-5 animate-in fade-in duration-300 xl:grid-cols-2">
          ...
          <div className="space-y-3">
            <textarea
              ...
              onChange={e => {
                ...
                setEditForm(prev => ({ ...prev, diagnosis: val }));
              }}
      ...
      <RecordBlock category="Support record" title="Behaviour plan and observations">
        {isEditing ? (
          <textarea
            ...
            onChange={e => {
              const val = e.target.value;
              setEditForm(prev => ({ ...prev, targetBehaviors: val }));
            }}
      ...
      <RecordBlock category="Medical record" title="Medical notes">
        {isEditing ? (
          <textarea
            value={editForm.medicalRecords || ''}
            onChange={e => {
              const val = e.target.value;
              setEditForm(prev => ({ ...prev, medicalRecords: val }));
            }}
            placeholder="Enter allergies, medication and medical notes..."
            className={recordInput}
            rows={5}
          />
        ) : (
          <p className="min-h-20 text-sm font-medium leading-6 text-slate-700 dark:text-slate-300">
            {student.medicalRecords || 'No medical notes have been recorded.'}</p>
        )}
      </RecordBlock>
      ...
      <RecordBlock category="Background record" title="Social and developmental history">
        {isEditing ? (
          <textarea
            ...
            onChange={e => {
              const val = e.target.value;
              setEditForm(prev => ({ ...prev, socialHistory: val }));
            }}
            placeholder="Enter relevant social and developmental history..."
            className={recordInput}
            rows={5}
          />
        ) : (
          <p className="min-h-20 text-sm font-medium leading-6 text-slate-700 dark:text-slate-300">
            {student.socialHistory || 'No social or developmental history has been recorded.'}</p>
        )}
      </RecordBlock>
    </div>
  );
